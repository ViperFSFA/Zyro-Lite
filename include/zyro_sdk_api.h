...
// ============================================================================
// Zyro-SDK public API. Shared between the firmware (Zyro-Lite_TDeck) and the
...
// gfx->, no SD.h, nothing). It only reaches the host through the ZyroApi
...
// Bump this if you ever change the struct layout below. Old .zApp binaries
// built against an older version are refused, not silently miscalled.
...
// RGB565 helper - apps build colors with this, same formula the firmware uses.
#ifndef ZYRO_RGB565
...
// let alone the firmware's own files.
struct ZyroFile {
...
struct ZyroSystem {
    ...
    // crashing. Never returns - the host unwinds and tears the app down.
    void     (*reportError)(const char *msg);
};

// ------------------------------------------------------------------ api ----
struct ZyroApi {
    uint32_t apiVersion; // == ZYRO_SDK_API_VERSION, checked by the host before entry is ever called
    ZyroCanvas canvas;
    ZyroFile   file;
    ZyroWifi   wifi;
    ZyroBle    ble;
